...
import {LocationConsumer, LocationProducer} from './infrastructure/kafka';
...
export class DependencyContainer {
	...
	async setupLocationObserver(): Promise<ObserverStartPoint> {
		const locationMediator = new LocationMediator(
			this.getCacheClient(),
			{locationApiUrl: this.dependencies.locationApiEndpoint, locationApiKey: this.dependencies.locationApiKey},
		);
